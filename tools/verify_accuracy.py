...
Checks each OPEN role in data/jobs.json against the rules the pipeline promises:
  ...
  5. it still reads as a mechanical internship (title-level filters)
...
def main() -> None:
    ...
    for r in open_jobs:
        ...
        if cfg.get("role_scope", "mechanical") != "all" and not filters.is_mechanical(
            title, include_general=bool(cfg.get("include_general_engineering", True))
        ):
