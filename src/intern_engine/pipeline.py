...
def _keep_matching(results, cfg, blocklist, existing=None) -> tuple[list, set[str], int, Counter]:
    ...
    scoped = cfg.get("role_scope", "mechanical") != "all"
    general_eng = bool(cfg.get("include_general_engineering", True))
    ...
    dropped_no_year = 0  # in-scope internships we skip only because the title has no year
    ...
    for company, result, error in results:
        ...
        for job in jobs:
            if not filters.is_internship(job.title):
                continue
            if scoped and not filters.is_mechanical(
                job.title, include_general=general_eng
            ):
                continue
...
def _close_out_of_scope(existing: dict, cfg: dict, blocklist: dict | None = None) -> int:
    ...
    a role alive forever after a blocklist, allowlist, role-scope, cycle, or
    ...
    for record in existing.values():
        ...
        rejected = (
            ...
            or (
                bool(title) and scoped
                and not filters.is_mechanical(title, include_general=general_eng)
            )
            or (
                has_cycle_evidence
