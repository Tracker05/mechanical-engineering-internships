...
Change behavior without touching code:
  ...
  - role_scope    : "mechanical" (design/manufacturing/thermal/automotive/aero/...)
                    or "all" internships.
  - include_general_engineering : keep discipline-free "Engineering Intern"
                    titles (default true). Set false for a stricter list.
"""
...
DEFAULTS = {
    ...
    "include_general_engineering": True,
}
...
_FALLBACK_REPO = "YOUR-GITHUB-USERNAME/Automated-List-Of-Mechanical-Engineering-Internships"
...
def validate_config(raw: object) -> dict:
    ...
    if cfg.get("role_scope") not in {"mechanical", "all"}:
        raise ConfigError("role_scope must be 'mechanical' or 'all'")
